...
async function demonstrateSyntax(): Promise<void> {
  console.log('═══════════════════════════════════════════════════════');
  ...
  const containerConfig: QuiltTypes.CreateContainerRequest = {
    // Basic configuration
    ...
    mounts: [
      {
        source: '/host/data',
        target: '/app/data',
        type: QuiltTypes.MountType.BIND,
        readonly: false
      },
      {
        source: 'app-logs',
        target: '/app/logs',
        type: QuiltTypes.MountType.VOLUME,
        readonly: false
      },
      {
        source: '',
        target: '/tmp',
        type: QuiltTypes.MountType.TMPFS,
        readonly: false,
        options: {
          size: '100m'
        }
  ...
  const volumesResponse: QuiltTypes.ListVolumesResponse =
  ...
  const dnsEntries: QuiltTypes.ListDnsEntriesResponse =
    await client.listDnsEntries({});
  ...
  const cleanupTasks: QuiltTypes.ListCleanupTasksResponse =
    await client.listCleanupTasks({});
...
demonstrateSyntax().catch((error: Error) => {
