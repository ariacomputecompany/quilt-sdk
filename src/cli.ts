...
import type { QuiltAuth, OperationAcceptedResponse } from "./types/common"
...
async function containerCreateCommand(ctx: CliContext, args: string[]): Promise<void> {
  ...
  const containerBody = withDefined({
    name: name ?? null,
    image,
    oci: oci ? true : null,
    environment: keyValueMap(arrayOption(parsed, "--env")),
    memory_limit_mb: numberOption(parsed, "--memory") ?? null,
    cpu_limit_percent: numberOption(parsed, "--cpu") ?? null,
    volumes: arrayOption(parsed, "--volume"),
    working_directory: optionalString(parsed, "--workdir") ?? null,
...
async function containerExecCommand(ctx: CliContext, args: string[]): Promise<void> {
  ...
  printResult(
    ctx,
    await ctx.client.containers.exec(
      identifier,
      {
      command,
      ...(workdir ? { workdir } : {}),
...
async function ociPullCommand(ctx: CliContext, args: string[]): Promise<void> {
  ...
  const pullBody: import("./modules/images").OciPullRequest = { reference }
  if (booleanOption(parsed, "--force")) {
    pullBody.force = true
  }
  ...
  printResult(
    ctx,
    await ctx.client.images.pull(pullBody),
  )
}
...
async function ociListCommand(ctx: CliContext, args: string[]): Promise<void> {
  ...
  printResult(
    ctx,
    await ctx.client.images.list(listQuery),
  )
}
